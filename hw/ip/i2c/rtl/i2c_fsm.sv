...
#(
  ...
  logic        acq_fifo_empty_or_last_space;
  logic        stretch_addr;
  logic        stretch_rx;
  logic        stretch_tx;
  logic        nack_timeout;
  ...
  assign acq_fifo_empty_or_last_space = acq_fifo_remainder <= AcqFifoDepthWidth'(1);
  ...
  always_comb begin : state_outputs
    ...
    unique case (state_q)
      ...
      NackHold : begin
        ...
        if (tcount_q == 20'd1) begin
          // ACQ FIFO should have space because stretching happens with two
          // spaces left in the FIFO: One for a stop/restart and the other for
          // this NACK message.
    ...
    unique case (state_q)
      ...
      AddrRead : begin
        if (bit_ack) begin
          if (address_match) begin
            if (acq_fifo_empty_or_last_space) begin
              // We must have stretched before, and software has been notified
              // through and ACQ FIFO full event. Now we must NACK
              // unconditionally for reads and for writes we should NACK the
              // next byte unconditionally.
              if (rw_bit_q) begin
                state_d = NackWait;
              end else begin
                state_d = AddrAckWait;
      ...
      AcquireByte : begin
        if (bit_ack) begin
          ...
          // completely.
          if (acq_fifo_empty_or_last_space || nack_next_byte_q) begin
            state_d = NackWait;
          end else begin
