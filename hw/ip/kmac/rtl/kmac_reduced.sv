...
#(
  ...
  // them.
  // SHA3 configuration
  input sha3_mode_e                    mode_i,      // e.g. ot_sha3_pkg::Sha3
  input keccak_strength_e              strength_i,  // e.g. ot_sha3_pkg::L256
  input logic [NSRegisterSize*8-1:0]   ns_prefix_i, // Ignored for Sha3,
                                                    // 48'h4341_4D4B_2001 for CShake
  input logic [ot_sha3_pkg::MsgStrbW-1:0] msg_strb_i,  // drive to all-1
  ...
  ///////////////////////////////////
  // Message packer FIFO
  logic [ot_sha3_pkg::MsgWidth-1:0] msg [NumShares];
  ...
  //////////////////////////
  logic msg_mask_en;
  logic [ot_sha3_pkg::MsgWidth-1:0] msg_mask, msg_mask_permuted;
  logic [ot_sha3_pkg::MsgWidth-1:0] msg_masked [NumShares];

  // Permute the PRNG output.
  always_comb begin
    msg_mask_permuted = '0;
    for (int unsigned i = 0 ; i < ot_sha3_pkg::MsgWidth ; i++) begin
  ...
  for (genvar i = 0; i < NumShares; i++) begin: gen_msg_masking
    assign msg_masked[i] =
        msg[i] ^ ({ot_sha3_pkg::MsgWidth{msg_mask_en_i}} & msg_mask_permuted);
  end
  ...
  // Error signals
  ot_sha3_pkg::err_t sha3_err, entropy_err;
