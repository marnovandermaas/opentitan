  ...
  virtual task check_keymgr_rsp_nonblocking();
    fork begin
      while (cfg.en_scb == 0 && entropy_fetched == 0) begin
        wait (cfg.m_kmac_app_agent_cfg[App0].vif.kmac_data_rsp.done == 1);
        if (cfg.enable_masking && kmac_err_type == ErrIncorrectEntropyMode) begin
          if (entropy_fetched == 0) begin
            `DV_CHECK_EQ(cfg.m_kmac_app_agent_cfg[App0].vif.kmac_data_rsp.error, 1)
          end
        end
        wait (cfg.m_kmac_app_agent_cfg[App0].vif.kmac_data_rsp.done == 0);
      end
    end join_none
  endtask
